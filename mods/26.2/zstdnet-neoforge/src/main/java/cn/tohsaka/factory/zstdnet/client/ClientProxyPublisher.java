...
public final class ClientProxyPublisher {
    ...
    private void onKeyPressed(ScreenEvent.KeyPressed.Pre event) {
        ...
        if (screen instanceof JoinMultiplayerScreen joinScreen) {
            if (!event.getKeyEvent().isSelection()) {
                return;
            }
        ...
        if (isShareToLanLikeScreen(screen)) {
            ...
            if (prepareLanWorldPublish(state)) {
                state.vanillaStartButton.onPress(new KeyEvent(InputConstants.KEY_RETURN, 0, 0));
            }
            event.setCanceled(true);
        }
    }
    ...
    private boolean connectEntry(Screen parent, ServerSelectionList.Entry entry) {
        ...
        if (entry instanceof ServerSelectionList.NetworkServerEntry networkEntry) {
            LanServer lanServer = readLanServer(networkEntry);
            if (lanServer == null) {
                return false;
            }
    ...
    /**
     * 26.1 起 {@code NetworkServerEntry#getServerData()} 被移除，仅保留 protected 字段 {@code serverData}
     * （类型 {@link LanServer}）。NeoForge 没有 mixin，且本变体未配置 AccessTransformer，故用反射读取该字段。
     */
    private static LanServer readLanServer(ServerSelectionList.NetworkServerEntry entry) {
        Class<?> type = entry.getClass();
        while (type != null) {
            for (Field field : type.getDeclaredFields()) {
                if (LanServer.class.isAssignableFrom(field.getType())) {
                    try {
                        field.setAccessible(true);
                        Object value = field.get(entry);
                        if (value instanceof LanServer lanServer) {
                            return lanServer;
                        }
                    } catch (ReflectiveOperationException | RuntimeException e) {
                        LOGGER.debug("zstdnet: failed to read LanServer field {}: {}", field.getName(), e.toString());
                    }
                }
            }
            type = type.getSuperclass();
        }
        LOGGER.warn("zstdnet: could not read NetworkServerEntry LanServer field via reflection.");
        return null;
    }
    ...
    private static boolean isShareToLanLikeScreen(Screen screen) {
        // 26.2 删除 ShareToLanScreen，「开放到局域网」界面并入 MultiplayerOptionsScreen（同一界面改名）。
        if (screen instanceof MultiplayerOptionsScreen) {
            return true;
        }
        String className = screen.getClass().getName().toLowerCase(Locale.ROOT);
        return className.contains("sharetolan") || className.contains("multiplayeroptions");
    }
