...
public final class ClientProxyPublisher {
    ...
    private final Map<MultiplayerOptionsScreen, ShareToLanState> shareToLanScreens = new WeakHashMap<>();
    ...
    public static EditBox onShareToLanInit(MultiplayerOptionsScreen screen) {
    ...
    public static void onShareToLanRender(MultiplayerOptionsScreen screen, GuiGraphicsExtractor guiGraphics) {
        INSTANCE.renderShareToLan(screen, guiGraphics);
    }

    public static void onShareToLanBeforeRender(MultiplayerOptionsScreen screen) {
    ...
    public static boolean onShareToLanKeyPressed(MultiplayerOptionsScreen screen, int keyCode) {
        return INSTANCE.handleShareToLanKeyPressed(screen, keyCode);
    }

    public static boolean onShareToLanMouseClicked(MultiplayerOptionsScreen screen, double mouseX, double mouseY, int button) {
    ...
    private void onClientTick(Minecraft minecraft) {
        ...
        // 26.2 把「当前屏幕」从 Minecraft.screen 字段搬到 Minecraft.gui（net.minecraft.client.gui.Gui）上的 screen() 方法。
    ...
    private void onRenderGui(GuiGraphicsExtractor gui) {
        ...
        // 26.2 移除 Options.hideGui；F1 隐藏 HUD 的状态搬到 Gui.hud（net.minecraft.client.gui.Hud）的 isHidden()。
    ...
    private EditBox attachShareToLanState(MultiplayerOptionsScreen screen) {
    ...
    private void handleScreenRemoved(Screen screen) {
        synchronized (stateLock) {
            ...
            } else if (screen instanceof MultiplayerOptionsScreen shareToLanScreen) {
                shareToLanScreens.remove(shareToLanScreen);
            }
        }
    }

    private void renderShareToLan(MultiplayerOptionsScreen shareToLanScreen, GuiGraphicsExtractor gui) {
    ...
    private void ensureShareToLanWidgetAttached(MultiplayerOptionsScreen shareToLanScreen) {
        ShareToLanState state = getShareToLanState(shareToLanScreen);
        if (state != null) {
    ...
    private boolean handleShareToLanKeyPressed(MultiplayerOptionsScreen screen, int keyCode) {
        if (keyCode != 257 && keyCode != 335) {
            return false;
        }
    ...
    private boolean handleShareToLanMouseClicked(MultiplayerOptionsScreen screen, double mouseX, double mouseY, int button) {
        if (button != 0) {
            return false;
        }
    ...
    private ShareToLanState getShareToLanState(MultiplayerOptionsScreen screen) {
        synchronized (stateLock) {
    ...
    private static boolean looksLikeVanillaLanPortEdit(EditBox editBox, MultiplayerOptionsScreen screen) {
        if (editBox == null) {
            return false;
        }
    ...
    private static EditBox findBackendPortEdit(List<?> listeners, MultiplayerOptionsScreen screen) {
        EditBox exact = null;
    ...
    private static void ensureShareToLanWidgetAttached(MultiplayerOptionsScreen screen, ShareToLanState state) {
