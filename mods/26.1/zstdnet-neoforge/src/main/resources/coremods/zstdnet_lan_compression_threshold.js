...
function initializeCoreMod() {
    return {
        ...
        'zstdnet_dedicated_auto_port': {
            ...
            'transformer': function(classNode) {
                var mappedInitServer = ASMAPI.mapMethod('initServer');
                var mappedGetProperties = ASMAPI.mapMethod('getProperties');

                for (var i = 0; i < classNode.methods.size(); i++) {
                    var method = classNode.methods.get(i);
                    if ((method.name != mappedInitServer && method.name != 'initServer') || method.desc != '()Z') {
                        continue;
                    }
                    ...
                    for (var insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
                        ...
                        if (insn.name != mappedGetProperties && insn.name != 'getProperties') {
                            continue;
                        }
