...
// 把 PlayerTabOverlay#render 里那处「连接是否加密」的门控（isLocalServer() || connection.isEncrypted()）
// 中的 Connection.isEncrypted() 调用替换为我方静态方法：对 ZstdNet 本地代理连接也判为安全，恢复 TAB 头像。
// 26.1 非混淆，方法名即 isEncrypted；仍按「owner=net/minecraft/network/Connection 且描述符 ()Z」定位
//（已核验 PlayerTabOverlay 内对 Connection 的唯一调用就是 isEncrypted）。
    ...
    return {
        'zstdnet_tab_player_head': {
            ...
            'transformer': function(classNode) {
                var connOwner = 'net/minecraft/network/Connection';
