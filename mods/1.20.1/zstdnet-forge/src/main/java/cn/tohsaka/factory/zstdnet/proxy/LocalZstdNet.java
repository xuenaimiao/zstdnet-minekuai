...
public final class LocalZstdNet {
    ...
    private static byte[] rewriteHandshakeDestination(byte[] handshakePayload, String host, int port) {
        ...
        String hostSuffix = ensureForgeHandshakeSuffix(extractHandshakeHostSuffix(originalHost));
        LOGGER.info(
            "zstdnet: rewriting handshake host original={} suffix={} presented={}:{}",
            sanitizeHandshakeHost(originalHost),
            sanitizeHandshakeHost(hostSuffix),
            host,
            port
        );
    ...
    private static String ensureForgeHandshakeSuffix(String hostSuffix) {
    ...
    private static String sanitizeHandshakeHost(String host) {
        if (host == null) {
            return "<null>";
        }
        return host.replace("\0", "\\0");
    }
