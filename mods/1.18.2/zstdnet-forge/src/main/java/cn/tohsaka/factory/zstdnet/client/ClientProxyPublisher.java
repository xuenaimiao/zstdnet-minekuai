...
public final class ClientProxyPublisher {
    ...
    private static final Component BACKEND_PORT_LABEL = new TranslatableComponent("zstdnet.share_to_lan.backend_port");
    private static final Component ZSTD_PORT_LABEL = new TranslatableComponent("zstdnet.share_to_lan.zstd_port");
    private static final Component ZSTD_PORT_INVALID = new TranslatableComponent("zstdnet.share_to_lan.port_invalid");
    private static final Component ZSTD_PORT_UNAVAILABLE = new TranslatableComponent("zstdnet.share_to_lan.port_unavailable");
    ...
    // 1.18.2 没有 ScreenEvent.Closing（1.19 才加入）；joinScreens/directJoinScreens/shareToLanScreens
    // 均为 WeakHashMap，屏幕关闭被 GC 回收后条目自动清除，因此无需 onScreenClosing。

    private void onScreenOpening(ScreenOpenEvent event) {
        // 1.18.2 的 ScreenOpenEvent 只暴露「即将打开的屏幕」，没有 getCurrentScreen()；
        // 当前屏幕从 Minecraft.screen 取（此时尚未切换，仍是旧屏幕）。
        Screen newScreen = event.getScreen();
        Screen currentScreen = Minecraft.getInstance().screen;
        if (newScreen instanceof ConnectScreen) {
            adoptInterceptedProxy();
        }
        if (currentScreen instanceof ConnectScreen && newScreen != null) {
    ...
    private void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        sendClientMessage(new TranslatableComponent("zstdnet.singleplayer.lan_hint"));
        sendClientMessage(new TranslatableComponent("zstdnet.singleplayer.lan_command_hint"));
    ...
    private void maybeShowRawFallbackNotice(Minecraft minecraft) {
        ...
        sendClientMessage(new TranslatableComponent(
            notice.knownRelay() ? "zstdnet.fallback.relay_notice" : "zstdnet.fallback.notice",
            notice.address()
        ));
        sendClientMessage(new TranslatableComponent("zstdnet.fallback.hint"));
    }
    ...
    private void onRenderGui(RenderGameOverlayEvent.Post event) {
        ...
        // 1.18.2 的 RenderGameOverlayEvent 用 getMatrixStack()（1.19 的 RenderGuiEvent 才改名 getPoseStack）。
        PoseStack gui = event.getMatrixStack();
        int y = 8;
    ...
    private void onScreenRender(ScreenEvent.DrawScreenEvent.Post event) {
        ...
        Minecraft.getInstance().font.drawShadow(poseStack, ZSTD_PORT_LABEL, labelX, labelY, 0xFFFFFF);
    }

    private void onScreenRenderPre(ScreenEvent.DrawScreenEvent.Pre event) {
        Screen screen = event.getScreen();
        if (isShareToLanLikeScreen(screen)) {
    ...
    private void onKeyPressed(ScreenEvent.KeyboardKeyPressedEvent.Pre event) {
        Screen screen = event.getScreen();
    ...
    private void onMousePressed(ScreenEvent.MouseClickedEvent.Pre event) {
    ...
    private boolean connect(Screen parent, ServerData serverData) {
        ...
        if (remote.directLan() && !ClientConfig.compressLan()) {
            ...
            ConnectScreen.startConnecting(parent, Minecraft.getInstance(), ServerAddress.parseString(remoteAddr), serverData);
            return true;
        }
        ...
        ConnectScreenHooks.setBypass(true);
        ConnectScreen.startConnecting(parent, Minecraft.getInstance(), ServerAddress.parseString(localAddr), serverData);
        return true;
    }
    ...
    private int showHudStatus(CommandContext<CommandSourceStack> context) {
        ...
        sendClientMessage(new TranslatableComponent("zstdnet.command.hud.status", visible ? "ON" : "OFF"));
        return 1;
    }
    ...
    private int setHudVisible(CommandContext<CommandSourceStack> context, boolean visible) {
        ...
        sendClientMessage(new TranslatableComponent(visible ? "zstdnet.command.hud.enabled" : "zstdnet.command.hud.disabled"));
        return 1;
    }
    ...
    private int setZstdPort(CommandContext<CommandSourceStack> context) {
        if (!canEditPortConfig()) {
            sendClientMessage(new TranslatableComponent("zstdnet.command.port.no_permission"));
            return 0;
        }
        ...
        try {
            ...
            sendClientMessage(new TranslatableComponent("zstdnet.command.port.write_failed"));
            return 0;
        }
        sendClientMessage(new TranslatableComponent(
    ...
    private int setVoicePort(CommandContext<CommandSourceStack> context) {
        if (!canEditPortConfig()) {
            sendClientMessage(new TranslatableComponent("zstdnet.command.voice.no_permission"));
            return 0;
        }
        ...
        try {
            ...
            sendClientMessage(new TranslatableComponent("zstdnet.command.voice.write_failed"));
            return 0;
        }
        sendClientMessage(new TranslatableComponent(
    ...
    private ServerData resolveDirectJoinServer(String remoteAddr) {
        ...
        // 1.18.2 的 ServerList 没有按 IP 查找的 get(String)（1.19 才加），手动遍历去重。
        for (int i = 0; i < serverList.size(); i++) {
            ServerData candidate = serverList.get(i);
            if (candidate != null && remoteAddr.equals(candidate.ip)) {
                return candidate;
            }
        }

        ServerData created = new ServerData(I18n.get("selectServer.defaultName"), remoteAddr, false);
        // 1.18.2 的 ServerList.add 只有单参（1.19 才加表示临时条目的 boolean）。
        serverList.add(created);
        serverList.save();
        return created;
    }
    ...
    private static int renderHudPanel(
        ...
        GuiComponent.fill(gui, x - 3, y - 3, x - 3 + width, y - 3 + height, backgroundColor);
        for (int i = 0; i < lines.length; i++) {
            int color = i == 0 ? titleColor : textColor;
            minecraft.font.drawShadow(gui, lines[i], x, y + lineHeight * i, color);
        }
        return y + height + 4;
    }
    ...
    private ShareToLanState attachShareToLanState(Screen screen, ScreenEvent.InitScreenEvent.Post event, List<?> listeners) {
        ...
        // 1.19.2 的 EditBox 没有 setHint/Tooltip API，端口校验错误只能用文字颜色提示。
    ...
    /** 1.19.2 的 HttpUtil 还没有 isPortAvailable（1.19.3 才加入），用本地绑定探测代替。 */
    ...
    private boolean prepareLanWorldPublish(ShareToLanState state) {
        ...
        try {
            ...
            sendClientMessage(new TranslatableComponent("zstdnet.share_to_lan.write_failed"));
            return false;
        }
        return true;
    }
