...
public final class ClientProxyPublisher {
    ...
    public static void onShareToLanRender(ShareToLanScreen screen, PoseStack poseStack) {
        INSTANCE.renderShareToLan(screen, poseStack);
    }
    ...
    private void onRenderGui(PoseStack gui, float tickDelta) {
    ...
    private EditBox attachShareToLanState(ShareToLanScreen screen) {
        ...
        // 1.19.3 起 AbstractWidget 的 x/y 字段改为 private，位置经 getX()/getY()/setX()/setY() 读写。
        ...
        // 1.19.3 的 EditBox 没有 setHint/Tooltip API，端口校验错误只能用文字颜色提示。
    ...
    private void renderShareToLan(ShareToLanScreen shareToLanScreen, PoseStack poseStack) {
    ...
    private boolean handleJoinKeyPressed(JoinMultiplayerScreen screen, int keyCode) {
        // 1.19.3 没有 CommonInputs：直接判回车 / 小键盘回车。
        return (keyCode == 257 || keyCode == 335) && connectSelected(screen);
    }
    ...
    private static int renderHudPanel(
        ...
        // 1.19.3 仍用 PoseStack 渲染：GuiComponent.fill 静态方法 + Font.drawShadow（GuiGraphics 是 1.20 才引入的）。
    ...
    private void applyZstdPortResponse(ShareToLanState state, String raw) {
        ...
        // 1.19.3 的 EditBox 没有 Tooltip API，端口校验错误只能用文字颜色提示。
