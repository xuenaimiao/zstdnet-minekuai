...
**语言 / Language：** 中文（当前） · [English](README.en-US.md)
...
- 当前仓库已同步支持：
  Forge 1.18.2、Forge 1.19.2、Forge 1.20.1、NeoForge 1.20.1、NeoForge 1.21.1、Fabric 1.20.1、Fabric 1.21.1、NeoForge 26.1、Fabric 26.1、NeoForge 26.2、Fabric 26.2
  （26.1 构建覆盖整个 26.1.x 线，即 26.1.1 / 26.1.2；26.2 构建覆盖 26.2.x；均需要 Java 25。NeoForge 26.2.x 目前仍为 beta）
...
- 也支持对接 authlib-injector / 自建 Yggdrasil（`premium_session_server` 支持逗号分隔多个基址，按序核验；服务端挂了 authlib-injector 时会自动探测皮肤站地址，LittleSkin / LinkSkin 等零配置可用）。
- 正版身份保护（`premium_uuid_guard`，默认开）：曾正版进服的玩家名若本次验证不通过（如启动器会话过期），会被拒绝并提示「请重新登录启动器」，而不是静默以离线 UUID 进服导致背包/进度「丢失」。

> **全部模组加载器已内置该功能**：Fabric、Forge、NeoForge（1.18.2 ~ 26.2，详见仓库 `PREMIUM_VERIFICATION.md`）。**Bukkit/Spigot 插件端暂不支持**（独立代理无法挂钩 MC 登录），插件端请仍把后端设 `online-mode=false`（如需保留正版身份可搭配 [TrueUUID](https://www.curseforge.com/minecraft/mc-mods/trueuuid)）。ZstdNet 不会在无法验证的加载器上擅自把后端改成离线。
>
...
- `chunk_cache`：区块引用缓存（CRC），面向大量重复区块 / 重访场景（默认：auto，**已默认开启**）
  - 在 ZSTD **之前**把客户端已持有的相同区块换成 8/16 字节令牌（或字节级增量），与 ZSTD 叠加压缩（详见「[区块引用缓存（CRC，默认开启）](#区块引用缓存crc默认开启)」）
  - `auto`（默认·推荐）/ `ref` / `full` / `measure`（只测量不改字节）/ `off`
  - 仅在客户端也支持并协商成功时对该连接生效；不支持 / 未升级的客户端逐字节透传，任何异常 `fail-closed` 断线重连，绝不发错包
  - 可缓存包表已覆盖全部发行变体（1.18.2 / 1.19.2 / 1.20.1 / 1.21.1 / 26.1）；其它未覆盖协议自动空操作、字节仍一致
...
- `raw_fallback`：服务端没有 ZstdNet 时是否自动回退原版直连（默认：true，**已默认开启**）
  - 点「加入服务器」时客户端先发一次 **zstd 压缩的 status 探测**：只有 ZstdNet 服务端能应答；
    对端不会说 ZSTD（典型场景：**樱花frp / OpenFrp 等联机映射把「开放到局域网」的原版端口直接映到公网**，
    端口后面并没有 ZstdNet 服务端）→ 自动改用**原版直连**进入，进服后聊天栏提示「本次连接未压缩」
  - 探测**连不上**（服务器离线 / 地址错误）→ 仍走 ZSTD 代理，保留原有的明确报错提示
  - 真挂在 frp 后面的 ZstdNet 服务器不受影响：探测能通过，照常压缩
  - 设为 `false` 恢复旧行为：强制 ZSTD，服务端没装时登录报错

- `compress_lan`：是否对局域网 / 本机 / 私网目标也启用压缩（默认：false，局域网走原版直连）
...
- `chunk_cache`：是否参与区块引用缓存 CRC（默认：true，**已默认开启**）
  - 开启后客户端会在握手里声明支持，并对「服务端→客户端」流装逆向解码器；仅当所连服务器也启用（`chunk_cache=auto/ref/full`）时该连接才真正生效，否则逐字节透传
  - 与 ZSTD 叠加：相同区块只回放本地缓存、不重收字节（详见「[区块引用缓存（CRC，默认开启）](#区块引用缓存crc默认开启)」）
- `chunk_cache_persist`：是否把整发区块落盘以支持跨会话 WARM_REF（默认：true）；关掉则仅会话内去重
- `chunk_cache_persist_mb`：跨会话缓存的磁盘 + 内存预算，每服务器，单位 MiB（默认：64）
...
> **原理来源**：本功能的 `full` / `ref` / `patch` 区块复用思路，**借鉴 / 参考**了开源项目
> [BandwidthOptimizer](https://github.com/duckgun13476/BandwidthOptimizer)（BO，GNU LGPL 2.1）。
> ZstdNet 的实现是**完全净室自研**，**未使用 BO 的任何代码**——线路格式、缓存策略、协商与
> `fail-closed` 规则均为本仓库独立设计与实现，仅在「思路 / 方向」上致敬其设计。

针对「**同一个区块被反复下发**」的场景（反复进出区块、跨维度往返、重连后重新加载——Create 系与大型
整合包尤为常见），ZSTD 的滑动窗口只能折叠「最近一段时间内」的重复；窗口外、或跨重连重现的同一区块，
仍要把整包重传一遍。区块引用缓存（CRC, Chunk Reference Cache）专门补这一块：

- **会话内 REF**：同一区块在本次游戏中再次下发时，只发 **8 字节引用令牌**，客户端重放本地缓存的原始字节。
- **跨会话 WARM_REF**：把整发过的区块落盘到本地；**重连后**仍持有的区块直接发 **16 字节令牌**，跨游戏会话省传。
- **字节级 PATCH**：区块只改了一点（如方块实体数据变化）时，发相对已持有基线的**增量**，而不是整块。
- 其余不可缓存的包**逐字节原样透传**。

### 与 ZSTD 是「叠加」不是「二选一」

CRC 工作在 **ZSTD 之前**：它先把能用令牌 / 增量代替的重复区块摘掉，**剩下的内容再交给 ZSTD 正常压缩**。
两者吃的是不同维度的冗余：

```text
后端原始包流
   │
   ▼  CRC：重复区块 → 8/16 字节令牌 / 小增量（这些字节根本不进 ZSTD）
   ▼  ZSTD：对剩下的全新区块 + 其它包做字节级压缩
   ▼
公网上线流量  =  ZSTD(全新内容)  +  一堆令牌
```

因此 CRC 与 ZSTD 的收益是**乘性叠加**：CRC 砍掉重复区块的「条数」，ZSTD 压缩剩余内容的「字节」。在区块
高度重复的服务器上，单靠 ZSTD 到不了的压缩比，借助 CRC 直接消除整块重传即可进一步压低。（CRC 与 ZSTD
之前的「实体包流变换」同处一个前置改写槽位，二者互斥、CRC 优先；而 ZSTD 始终是兜底底座，即使前置层都不
生效，连接也与历史行为逐字节一致。）

### 默认开启，但只在协商成功时才动字节

和 LDM / 字典 / 实体变换（默认关闭）不同，CRC **默认开启**（服务端 `chunk_cache=auto`，客户端
`chunk_cache=true`），开箱即用就是 **ZSTD + CRC 叠加**，无需任何手动配置。但「默认开」不等于「无条件改
字节」——它只在以下条件全部满足时才对某条连接生效：

- 客户端也安装并启用了 ZstdNet（握手时声明支持）；
- 当前 MC 协议有内置可缓存包表（已覆盖全部发行变体：1.18.2 / 1.19.2 / 1.20.1 / 1.21.1 / 26.1；其它未覆盖协议 → 该连接自动空操作）；
- 该连接未使用训练字典。

任一不满足 → 该连接**逐字节透传**，与未启用本功能时完全一致；运行中任何异常 → **`fail-closed` 断开并由
原版自动重连，绝不发错包**。正确性不依赖内置 packet 表：表只用于编码端决定「哪些区块值得缓存」，表缺 /
不符只会少压、绝不损坏数据。这正是默认敢开的原因：不支持的客户端 / 版本自动退化，没有兼容风险。

### 模式与配置

服务端 `chunk_cache`（`config/zstdnet-server.properties`）：

- `auto`（**默认·推荐**）：协商成功即自动启用 REF + 跨会话 WARM_REF + 自适应旁路，划算时叠加字节级 PATCH；
  遇连续不划算会临时旁路并周期性重探。
- `ref`：只做确定性的区块引用去重（REF + WARM_REF），不发 PATCH、不自适应旁路。
- `full`：REF + WARM_REF + PATCH（确定性，不旁路）。
- `measure`：**只测量不改字节**——统计「可去重的重复区块流量」以及「现有 ZSTD/LDM 窗口已折叠 vs 还剩多少
  边际空间」，作为是否上 CRC 的决策依据。
- `off`：完全关闭，逐字节与历史一致、零开销。

客户端 `config/zstdnet-client.toml`：

- `chunk_cache`（默认 true）：是否参与区块引用缓存。
- `chunk_cache_persist`（默认 true）：是否把整发区块落盘以支持跨会话 WARM_REF；关掉则仅会话内 REF。
- `chunk_cache_persist_mb`（默认 64）：跨会话缓存的磁盘 + 内存预算（每服务器，单位 MiB）。

> 提示：本功能为较新增项，已有字节级正确性单测兜底、并对不支持场景 `fail-closed`；上生产前建议先用
> `chunk_cache=measure` 观察实际收益，或小范围灰度后再全量启用。
...
- **区块引用缓存（`chunk_cache`，默认开启）**：在 ZSTD 之前把客户端已持有的相同区块换成 8/16 字节令牌（会话内 REF / 跨会话 WARM_REF）或字节级增量（PATCH），消除窗口外与跨重连的整块重传，与 ZSTD **叠加**进一步压低带宽；协商生效、默认 `auto`、`fail-closed` 不损坏数据。其 `full`/`ref`/`patch` 思路**借鉴参考** [BandwidthOptimizer](https://github.com/duckgun13476/BandwidthOptimizer)（LGPL 2.1），但**净室自研、未使用其代码**
...
「区块引用缓存（CRC）」功能的 `full` / `ref` / `patch` 区块复用**设计思路借鉴 / 参考**了开源项目
[BandwidthOptimizer](https://github.com/duckgun13476/BandwidthOptimizer)（GNU LGPL 2.1）。ZstdNet 的实现为
**完全净室自研、未使用其任何代码**，仅在思路与方向上致敬其设计；本仓库实现的著作权与许可归本项目所有。
